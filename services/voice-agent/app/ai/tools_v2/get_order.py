...
from ..common.validators import clean_order_number
...
def _check_cancel_eligibility(
    ...
    This function has NO dependencies on external services and can be
    unit-tested in isolation.
    """
    # Rule 1 — already cancelled / refunded
    ...
    # Rule 3 — partial shipment (some items out the door)
    ...
    # Rule 4 — nothing shipped yet, standard cancellation path
    ...
    # Rule 5 — unclear state, always safer to involve a human
    return CancelEligibility(
        can_cancel=False,
        reason=(
...
def _format_voice_summary(order: GetOrderResponseData) -> str:
    ...
    Rules:
    - Never exceed ~40 words (phone attention span)
    - Always mention order number for caller confirmation
    - Include actionable information (tracking, cancel option)
    - Never say "null", "None", or raw status codes
    """
    ...
    if status == "fulfilled":
        ...
        if tracking:
            # Read tracking as spaced-out digits — easier to hear
...
#
# Scenarios are chosen by the LAST DIGIT of the order number:
...
#   9    → Order not found
#
# This covers every voice-summary and cancel-eligibility branch.
# ─────────────────────────────────────────────────────────────────────────────
...
class MockOrderRepository:
    ...
    Replace this class — and the call in _resolve_order() — with
    ShopifyOrderClient once the API is ready.
    """
    ...
    def get(order_number: str) -> GetOrderResponseData:
        ...
        # ── Partially fulfilled / requires human ──────────────────────────
        return GetOrderResponseData(
            found=True,
            order_number=order_number,
            order_id=f"mock_{order_number}",
...
#
# When you're ready to connect the real Shopify Admin REST API:
#   1. Implement ShopifyOrderClient.get() using common/shopify.py
#   2. Map the raw Shopify order dict to GetOrderResponseData
#   3. Set SHOPIFY_USE_MOCK=false in the agent config or env
#   4. Delete MockOrderRepository
# ─────────────────────────────────────────────────────────────────────────────


class ShopifyOrderClient:
    """
    Interface contract for the real Shopify Admin API client.

    NOT YET IMPLEMENTED.  _resolve_order() below calls MockOrderRepository
    ...
    async def get(self, order_number: str) -> GetOrderResponseData:
        # TODO: implement using common/shopify.ShopifyAdminClient
        #   raw = await ShopifyAdminClient(self._domain, self._access_token)
        #              .get_order(order_number)
        #   if not raw:
        #       return GetOrderResponseData(found=False, order_number=order_number)
        #   return _map_shopify_order(raw)
...
def _map_shopify_order(raw: dict[str, Any]) -> GetOrderResponseData:
    """
    Map a raw Shopify Admin API order dict to GetOrderResponseData.

    NOT YET ACTIVE — called only by ShopifyOrderClient.get().
    Keeping it here so the mapping logic is visible and reviewable.
    """
    line_items = [
        OrderItem(
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 7 — Data resolver  (mock vs real switchboard)
# ─────────────────────────────────────────────────────────────────────────────
...
async def _resolve_order(
    ...
    Currently: always uses mock (ShopifyOrderClient raises NotImplementedError).
    When real API is ready: remove the try/except and let ShopifyOrderClient run.
    """
...
class GetOrderTool(BaseTool):
    ...
    async def execute(self, args: dict[str, Any], context: ToolContext) -> ToolResult:
        ...
        # ── 3. Apply cancel-eligibility rules ─────────────────────────────────
        if order.found:
        ...
        return ToolResult(
            success=True,  # tool ran correctly — found/not-found is in data.found
            data={
                "success": True,
                "message": message,
