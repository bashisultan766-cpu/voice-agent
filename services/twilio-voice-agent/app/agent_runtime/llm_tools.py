...
async def dispatch(name: str, args: dict, session: "SessionState | None") -> str:
    ...
    try:
        ...
        if name in ("lookup_shopify_order_details", "get_order_details", "lookup_order"):
            from ..tools.shopify_tools import customer_facing_order_tool_json

            out = customer_facing_order_tool_json(out)
        return out
