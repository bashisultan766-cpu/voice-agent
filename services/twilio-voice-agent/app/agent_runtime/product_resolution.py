...
Single lookup pipeline: match_product → exact + similar hits, or similarity_engine
fallback — no LLM catalog invention.
"""
...
from .workflow_contracts import PRODUCT_SEARCH_WORKFLOW, workflow_guard

logger = logging.getLogger(__name__)

PRODUCT_RESOLUTION_VERSION = "v1.0"
    ...
    *,
    ...
    if isbn:
        ...
        if isinstance(payload, dict) and payload.get("found") and payload.get("product"):
            ...
            resolution.exact_score = 100.0
            resolution.product_kind = _infer_product_kind(exact, query)
            catalog_hits = await _load_catalog_hits(
                session, product.get("title") or query, tool_results=resolution.tool_results,
            )
            resolution.similar = _pick_similar(
                query,
                catalog_hits,
                exact=exact,
    ...
    if scored:
        ...
        if top_score >= _EXACT_TITLE_SCORE:
            ...
            resolution.similar = _pick_similar(
                query,
                [hit for _, hit in scored],
                exact=resolution.exact,
                product_kind=resolution.product_kind,
            )
        else:
            resolution.similar = _dedupe_hits([hit for _, hit in scored])[:_MAX_SIMILAR]

    if not resolution.similar:
        catalog_pool = [hit for _, hit in scored] if scored else []
        resolution.similar = similarity_engine(
            session,
            query,
            product_kind=resolution.product_kind,
            exclude_variant_ids={
                str(resolution.exact.get("variant_id") or "")
            } if resolution.exact else None,
            catalog_hits=catalog_pool,
        )
    return resolution
...
async def _load_catalog_hits(
    ...
    *,
    ...
    if isinstance(payload, dict):
        tool_results.append(("catalog_search", payload))
    results = (payload.get("results") or []) if isinstance(payload, dict) else []
    return [normalize_catalog_hit(item) for item in results if isinstance(item, dict)]
...
def _pick_similar(
    ...
    *,
    ...
    exact_vid = str((exact or {}).get("variant_id") or "")
    scored: list[tuple[float, dict[str, Any]]] = []
    for hit in hits:
        vid = str(hit.get("variant_id") or "")
        if exact_vid and vid == exact_vid:
            continue
        if product_kind and _infer_product_kind(hit, query) != product_kind:
            continue
        score = _score_hit(query, hit)
        if score >= _SIMILAR_TITLE_SCORE:
            scored.append((score, hit))
    scored.sort(key=lambda item: item[0], reverse=True)
    return _dedupe_hits([hit for _, hit in scored])[:_MAX_SIMILAR]
...
def format_exact_match_reply(resolution: ProductResolution) -> str:
    from .commerce_flow_state import _price_phrase, _title, quantity_prompt

    exact = resolution.exact or {}
    if exact.get("variant_id"):
        base = quantity_prompt(exact)
    else:
        base = f"I found {_title(exact)}. {_price_phrase(exact)}"
...
def format_no_exact_reply(resolution: ProductResolution) -> str:
    ...
    return f"We couldn't find the exact product for {query}."
...
def _similar_titles(
    ...
    *,
    ...
    from .commerce_flow_state import _title

    exclude_vid = str((exclude or {}).get("variant_id") or "")
    names: list[str] = []
    for hit in hits[:_MAX_SIMILAR]:
        if exclude_vid and str(hit.get("variant_id") or "") == exclude_vid:
            continue
        name = _title(hit)
...
def has_good_similarity_match(resolution: ProductResolution) -> bool:
    """True when catalog returned scored alternatives worth offering."""
...
async def product_resolution_to_short_circuit(
    ...
    *,
    ...
    if resolution.exact:
        ...
        if not catalog_hit_is_orderable(top):
            ...
            msg = support_handoff_preparation(
                ...
                product_title=title,
                alternatives=resolution.similar,
            )
    ...
    record_product_candidate(session, title=(resolution.query or "")[:80], found=False)
    ...
    stage_product_search_fallback(
        ...
        query=resolution.query or isbn,
        isbn=isbn,
        escalation_eligible=not has_similar,
    )
    return IsbnShortCircuitResult(
