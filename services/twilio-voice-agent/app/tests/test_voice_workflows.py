...
def test_detect_intent_without_identifier():
    session = _session()
    assert detect_product_search_intent(session, "I want a book")
    assert requires_product_clarification(session, "I want a book")


def test_bare_title_requires_clarification_not_search():
    session = _session()
    assert detect_product_search_intent(session, "Game of Thrones")
    assert requires_product_clarification(session, "Game of Thrones")
    assert not has_valid_product_identifier(session, "Game of Thrones")
