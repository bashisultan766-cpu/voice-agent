...
describe("SHOSHAN_SYSTEM_PROMPT anti-hallucination", () => {
  ...
  it("authorizes order money fields for unverified callers while locking vault fields", () => {
    ...
    expect(SHOSHAN_SYSTEM_PROMPT).toMatch(/9, 4, 4, 9/);
    expect(SHOSHAN_SYSTEM_PROMPT).toMatch(/COMMA PACING|comma\+space/i);
  });
  ...
  it("requires Concierge Gateway for initial order lookup", () => {
    ...
    expect(SHOSHAN_SYSTEM_PROMPT).toMatch(/STRICT CONVERSATIONAL ECONOMY|CONVERSATION LOCK/i);
    expect(SHOSHAN_SYSTEM_PROMPT).toMatch(/Never act like a database|FORBIDDEN Behavior|NEVER read the tracking number/i);
    expect(SHOSHAN_SYSTEM_PROMPT).toMatch(
      /I have successfully pulled up order \[Number\] for \[Customer Name\]\. Order status is \[Status\]/i,
    );
    expect(SHOSHAN_SYSTEM_PROMPT).toMatch(/How can I assist you further with this order/i);
