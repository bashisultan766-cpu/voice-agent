...
describe("streamHandler dictation interrupt integration", () => {
  ...
  it("applyRelayInterrupt is ignored when agent is not speaking", () => {
  ...
  it("pre-emptively aborts dictation and signals orchestrator on interrupt", async () => {
    ...
    while (hoisted.mockProcess.mock.calls.length < 2 && Date.now() < deadline) {
      await new Promise((resolve) => setTimeout(resolve, 20));
    }

    expect(hoisted.mockProcess).toHaveBeenCalledWith(
      CALL_SID,
      USER_INTERRUPTED_DICTATION_SIGNAL,
      expect.anything(),
    );
