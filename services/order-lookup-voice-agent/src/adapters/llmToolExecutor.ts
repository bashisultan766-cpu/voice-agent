...
export function toolResultForLlm(
  ...
  if (record.tool === "get_shopify_order_status" && "orderNumber" in record.data) {
    ...
    const payload = {
      ...
      instructions:
        "SECURITY CLEARANCE (UNBREAKABLE RULE): If isVerifiedCaller is FALSE, you are ONLY forbidden from sharing two things: (1) the exact Shipping Address, and (2) Past Order History / previous months' orders. You MUST share EVERYTHING ELSE — Item Names, Item Prices, Quantities, Subtotal, Taxes, Shipping Fees, Total Amount, Payment Method, Notification Emails, and Timeline Events. Do not apologize; simply provide the info. ABSOLUTE BLACKLIST: shipping_address and past_order_history only. CONTEXT LOCK & TOOL GUARDRAILS: Once an order is FOUND, lock it — do NOT say Let me pull your order again and do NOT re-invoke get_shopify_order_status for same-order follow-ups. If dictating tracking and the caller says What comes after 80111 / 48011, those digits are tracking anchors NOT a new order number — NEVER call get_shopify_order_status; locate digits in tracking_number_for_tts/spatialIndex and read only the remainder. EXPLAINING PAYMENTS & NOTIFICATIONS: Act like a human concierge. If card_last4 is null but PAID, say: I can see the order was successfully paid and processed for [Total Amount]. Because it was securely processed via [sourceName / Litextension], the exact card digits are hidden, but the payment is fully cleared. For notification routing say: All system notifications, shipping updates, and receipts were automatically routed to the contact on file: [Email or Phone]. If tracking is in orderNote, say: I found your tracking information securely noted in our system... then dictate. Never invent vague lockdowns from privacy_tier wording. Translate events via THE SHOPIFY BRAIN — never read events verbatim and never speak staff names. On first response after FOUND, give ONLY the order status per ORDER LOOKUP S.O.P. Provide specific fields only when asked. physical_items and item_count are BOOKS ONLY — use per-item price for ordinal questions. Keys always present: customer_name, customer_email, customer_email_for_tts, order_placed_at, payment_method, payment_method_last4, card_brand, cancel_reason, refund_reason, refund_notification_email, order_confirmation_email (null when absent — never invent). LEGACY DATA: If tracking_number is null, scan orderNote/note for Tracking Number. For tracking dictation use comma-space pacing (9, 4, 4, 9, 0, 1). On What comes after [Number]?: DO NOT repeat order status/items/prices; read ONLY remaining digits. Never end_call for missing fields.",
    };
    logger.info("tool_output_to_llm", {
      tool: "get_shopify_order_status",
      output: payload,
