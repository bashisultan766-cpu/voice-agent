...
/**
 * Full deep order status lookup — single Shopify entry point.
 * Fuzzy STT candidates are tried inside getOrderStatus; do not retry not_found here.
 */
export async function lookupOrderStatus(
  ...
  const candidates = fuzzyOrderNumberCandidates(rawOrderNumber);
  const orderNumber = candidates[0] ?? normalizeOrderNumber(rawOrderNumber);
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    ...
    try {
      const data = await withTimeout(
        getOrderStatus(rawOrderNumber, callSid),
      ...
      // Never retry not_found — fuzzy candidates already ran inside getOrderStatus.
