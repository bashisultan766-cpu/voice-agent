...
export async function ensureShopifyProductScopes(): Promise<void> {
  ...
  const scopesRes = await fetchWithTimeout(
    `${shopOrigin()}/admin/oauth/access_scopes.json`,
    cfg.SHOPIFY_TIMEOUT_MS,
  );

  if (!scopesRes.ok) {
    const body = await scopesRes.text();
    logger.warn("shopify_scope_check_failed", { status: scopesRes.status, body: body.slice(0, 120) });
    scopesVerified = true;
    return;
  }
