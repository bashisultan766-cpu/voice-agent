 ...
 * Shopify search isolation — fingerprinted queries, circuit-breaker guarded.
 ...
 * Execute Shopify work behind circuit breaker.

 * Never retries THROTTLED — circuit opens immediately.

 * At most one retry for transient non-throttle errors.

 */

async function withShopifyApiGuard<T>(
  ...
  return withShopifyCircuitBreaker(callSid, operation, async () => {
    ...
    try {
      ...
      if (isShopifyThrottleError(firstErr)) throw firstErr;
...
export async function searchProductByTitleIsolated(
  ...
  let products = await withShopifyApiGuard(callSid, "title_search", () => truthSearchByTitle(q));
...
export async function searchProductByISBNIsolated(
  ...
  let products = await withShopifyApiGuard(callSid, "isbn_search", () => truthSearchByIsbn(isbn));
