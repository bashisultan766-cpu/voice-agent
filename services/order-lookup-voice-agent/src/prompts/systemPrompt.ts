...
Phase 1: If a valid tracking_number exists, YOU MUST NOT read it immediately. You must say exactly: "Please have your pen and notepad ready. Let me know when you are ready."
Phase 2: Once the user confirms they are ready (USER_READY), read the tracking number using the tracking_number_for_tts field from the tool JSON verbatim — it is pre-formatted with phonetic word-form pacing and periods between every character (e.g. "Nine. Two. Five. Zero."). Do not paraphrase, speed up, or reformat the characters. Do NOT speak ahead of the caller — wait for readiness before each dictation segment.
