/**
 * Voice utterance → Shopify product search query cleanup and probable title extraction.
 */

const FILLER_PHRASES =
  /\b(i\s+need|i\s+want|i'?m\s+looking\s+for|looking\s+for|show\s+me|can\s+you\s+get|give\s+me|do\s+you\s+have\s+it|do\s+you\s+have|could\s+you\s+check|is\s+it\s+available|got\s+any|uh|um|er|ah|like|please|thanks|thank\s+you)\b/gi;

/** Whole-word filler; "book" removed so titles like "The Blue Book" still work after other cleanup. */
const BOOK_WORD = /\bbooks?\b/gi;

const TRAILING_TAIL = /\b(it|that|this|one|them|those|something|anything)\s*[.?!,]*\s*$/i;

function extractIsbnDigitsFromText(s: string): string | null {
...
/**
 * Strip noise and extract the most likely product title phrase for search + ranking.
 * Example: "I need the book Mind Battles. Do you have it?" → probableTitle "Mind Battles"
 */
export function cleanVoiceProductQuery(raw: string): { cleanedQuery: string; probableTitle: string } {
  let s = `${raw ?? ''}`.trim();
  if (!s) return { cleanedQuery: '', probableTitle: '' };
...
export interface ShopifySearchAttempt {
  ...
  label: string;
  /** Shopify Admin `products(query:)` search string. */
  query: string;
}

/**
 * Multi-strategy Shopify `products(query:)` attempts.
 * Caller runs **all** attempts and merges results (do not stop early).
 *
 * Order: title:"…", "…", fuzzy, handle, sku (title text), ISBN/barcode, tags, vendor/product_type.
 */
export function buildShopifyProductSearchAttempts(input: {
  probableTitle: string;
  cleanedQuery: string;
  /** Original user text (for ISBN embedded in the utterance). */
  ...
  const attempts: ShopifySearchAttempt[] = [];
  ...
  if (pt.length >= 2) {
    ...
    if (handle.length >= 3) push('4_handle', `handle:${handle}`);

    push('5a_sku_title', `sku:${safe}`);
    if (safe.includes(' ')) push('5b_sku_title_quoted', `sku:"${safe}"`);
