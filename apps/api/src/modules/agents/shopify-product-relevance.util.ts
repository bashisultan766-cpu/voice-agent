/**
 * Local re-ranking for Shopify Admin `products` search — Shopify relevance order is often too broad for voice.
 */

/** Scores below this are weak matches and are ignored for voice. */
export const PRODUCT_SEARCH_MIN_CONSIDER_SCORE = 600;

/** Below this, do not present a product to the customer. */
export const PRODUCT_SEARCH_CONFIRM_MIN_SCORE = 650;

/** At or above: confident price/stock answer (all words in title or better). */
export const PRODUCT_SEARCH_CONFIDENT_MIN_SCORE = 800;

/** @deprecated Use PRODUCT_SEARCH_MIN_CONSIDER_SCORE */
export const PRODUCT_RELEVANCE_SCORE_THRESHOLD = PRODUCT_SEARCH_MIN_CONSIDER_SCORE;
...
export interface RankableCatalogProduct {
  ...
  tags?: string[] | null;
  isbn?: string | null;
  variants: Array<{ sku?: string | null; isbn?: string | null; barcode?: string | null }>;
}
...
function normalizeIsbnDigits(value: string): string {
  return value.replace(/[^0-9Xx]/g, '').toUpperCase();
}

function normalizeSkuBarcode(s: string): string {
  return s
    .toLowerCase()
    .normalize('NFKD')
    .replace(/\p{M}/gu, '')
...
function queryWordsFromNormalized(normalized: string): string[] {
  return normalized
    .split(' ')
    .map((w) => w.trim())
...
function extractQueryIsbn(userQuery: string): string | null {
  const digits = userQuery.replace(/[^0-9Xx]/g, '');
  if (digits.length === 10 || digits.length === 13) return digits.toUpperCase();
  return null;
}

function tagsBlob(tags: string[] | null | undefined): string {
  return normalizeForMatch((tags ?? []).join(' '));
}

/**
 * Voice scoring (strict):
 * - **1000** — normalized product title equals probable/query title
 * - **900** — exact SKU, barcode, or ISBN match
 * - **800** — every probable-title word appears in the title
 * - **750** — full probable-title phrase appears in the title (or all words in tags)
 * - **<600** — weak partial overlap → treated as no match for surfacing
 */
export function scoreCatalogProduct(
  queryOriginal: string,
  probableTitle: string,
  product: RankableCatalogProduct,
  ...
  if (probableNorm.length > 0 && titleNorm === probableNorm) return { score: 1000, matchReason: 'title_exact_probable' };
  if (queryNorm.length > 0 && titleNorm === queryNorm) return { score: 1000, matchReason: 'title_exact_query' };

  if (queryIsbn) {
    const pIsbn = product.isbn ? normalizeIsbnDigits(product.isbn) : '';
    if (pIsbn && pIsbn === queryIsbn) return { score: 900, matchReason: 'isbn_exact_product' };
    for (const v of product.variants) {
      const visbn = v.isbn ? normalizeIsbnDigits(v.isbn) : '';
      if (visbn && visbn === queryIsbn) return { score: 900, matchReason: 'isbn_exact_variant' };
      const skuDigits = (v.sku ?? '').replace(/[^0-9Xx]/g, '').toUpperCase();
      if (skuDigits && skuDigits === queryIsbn) return { score: 900, matchReason: 'sku_isbn_exact' };
      const bcDigits = (v.barcode ?? '').replace(/[^0-9Xx]/g, '').toUpperCase();
      if (bcDigits && bcDigits === queryIsbn) return { score: 900, matchReason: 'barcode_isbn_exact' };
    }
  }

  const matchSkuOrBarcode = (raw: string | null | undefined): boolean => {
    const r = (raw ?? '').trim();
    if (!r) return false;
    ...
    if (probableNorm && compact === normalizeSkuBarcode(probableNorm)) return true;
    if (queryNorm && compact === normalizeSkuBarcode(queryNorm)) return true;
    return false;
  };

  for (const v of product.variants) {
    if (matchSkuOrBarcode(v.sku)) return { score: 900, matchReason: 'sku_exact' };
    if (matchSkuOrBarcode(v.barcode)) return { score: 900, matchReason: 'barcode_exact' };
  }

  if (words.length > 0) {
    const allInTitle = words.every((w) => titleNorm.includes(w));
    if (allInTitle) {
      return { score: PRODUCT_SEARCH_CONFIDENT_MIN_SCORE, matchReason: 'all_title_words_in_product_title' };
    }
...
export function rankCatalogProductsForVoice<T extends RankableCatalogProduct>(
  queryOriginal: string,
  probableTitle: string,
  products: T[],
  maxResults: number,
): {
  ranked: Array<T & { relevanceScore: number; matchReason: string }>;
  rankedForLog: Array<{ title: string; score: number; matchReason: string }>;
  bestScore: number;
  bestReason: string | null;
  lowConfidence: boolean;
  productsAfterRanking: number;
  /** Best-scoring merged candidate title (even if below voice thresholds). */
  ...
  const productsAfterRanking = scored.filter((p) => p.relevanceScore >= PRODUCT_SEARCH_MIN_CONSIDER_SCORE).length;
