...
import {
  ...
  minimalWebhookPayload as buildMinimalWebhookPayload,
} from './webhook-reconciliation.util';
...
export class ShopifyService {
  ...
  private shouldStoreFullWebhookPayload(): boolean {
    return this.config.get<string>('SHOPIFY_STORE_FULL_WEBHOOK_PAYLOAD') === 'true';
  }

  private minimalWebhookPayload(topic: string, payload: unknown) {
    const p = payload as {
      id?: number | string;
      name?: string;
    ...
    return buildMinimalWebhookPayload(topic, {
      ...p,
  ...
  private paymentStatusFromOrder(
    ...
    if (status === 'voided' || status === 'refunded') return 'FAILED';
    if (status === 'expired' || cancelledAt || closedAt) return 'EXPIRED';
    return 'PENDING';
  }

  private checkoutStatusFromPaymentStatus(
    status: 'PENDING' | 'PAID' | 'FAILED' | 'EXPIRED',
  ): 'OPENED' | 'COMPLETED' | 'FAILED' | 'EXPIRED' {
  ...
  private async findCheckoutLinkForOrder(tenantId: string, email: string | null, orderId: string) {
  ...
  private async reconcileOrderWebhookForTenant(
    tenantId: string,
    topic: string,
    domain: string,
    payload: unknown,
    ...
    const paymentStatus = this.paymentStatusFromOrder(
      p.financial_status ?? null,
      p.cancelled_at ?? null,
      p.closed_at ?? null,
    );
    ...
    const webhookEventKey = buildPaymentWebhookEventKey(topic, orderId, tenantId, checkout.id);
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.paymentRecord.upsert({
        ...
        update: {
          ...
          rawWebhookPayloadJson: this.shouldStoreFullWebhookPayload()
            ...
            : (this.minimalWebhookPayload(topic, payload) as Prisma.InputJsonValue),
          metadata: {
      ...
      await tx.checkoutLink.updateMany({
        ...
        data: {
          ...
          metadata: {
            ...(typeof checkout.metadata === 'object' && checkout.metadata ? (checkout.metadata as Record<string, unknown>) : {}),
            paymentStatus,
            shopifyOrderId: orderId,
            shopifyOrderName: p.name ?? null,
            lastWebhookTopic: topic,
  ...
  async handleWebhook(topic: string, shopDomain: string, payload: unknown): Promise<void> {
    ...
    if (topic === 'orders/create' || topic === 'orders/updated') {
      await Promise.all(
        [...tenantIds].map((tenantId) =>
          this.reconcileOrderWebhookForTenant(tenantId, topic, domain, payload).catch(async (err) => {
            ...
            await this.prisma.auditLog.create({
              data: {
                ...
                entityType: 'SHOPIFY_ORDER',
                entityId: String((payload as { id?: number | string })?.id ?? 'unknown'),
                metadata: {
                  topic,
                  shopDomain: domain,
                  message,
