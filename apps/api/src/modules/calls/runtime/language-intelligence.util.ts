...
export function detectLanguageFromText(text: string): LanguageDetectionResult {
  ...
  if (/[ء-ي]/.test(t)) return { language: 'ar', confidence: 0.97 };
  if (/[ঀ-৿]/.test(t)) return { language: 'bn', confidence: 0.97 };
  if (/[ऀ-ॿ]/.test(t)) return { language: 'hi', confidence: 0.97 };
  if (/[ぁ-ゟ゠-ヿ]/.test(t)) return { language: 'ja', confidence: 0.97 };
  if (/[가-힯]/.test(t)) return { language: 'ko', confidence: 0.97 };
  if (/[一-龯]/.test(t)) return { language: 'zh', confidence: 0.97 };
...
export function normalizeLanguageForTwilio(language: string | null | undefined): string {
