...
export class CallMemoryService {
  ...
  summarizeForPrompt(memory: CallConversationMemory): string {
    ...
    if (memory.priceSensitivity) lines.push(`Price sensitivity: ${memory.priceSensitivity}.`);
    if (memory.purchaseUrgency && memory.purchaseUrgency !== 'low') {
      lines.push(`Purchase urgency: ${memory.purchaseUrgency}.`);
    }
    if (memory.interestSignals?.length) {
      lines.push(`Interest themes: ${memory.interestSignals.join(', ')}.`);
    }
    if ((memory.recommendationDeclined ?? 0) > 0) {
      lines.push(`Declined recommendations: ${memory.recommendationDeclined}.`);
    }
