...
function looksLikeQuestion(text, intent) {
    const t = normalizeText(text);
    if (intent === 'product_question' || intent === 'payment_question')
        return true;
    if (t.includes('?'))
        return true;
    return /\b(what|which|how|when|where|why|price|cost|available|payment)\b/i.test(t);
}
function parseKnownProductFromAssistantText(text) {
...
function getKnownProductFromToolResult(toolResult) {
    const sp = toolResult?.searchProducts;
    if (!sp?.ok || !sp.found || !sp.title)
        return null;
    return { title: sp.title, price: sp.price ?? undefined };
}
function getKnownProductFromHistory(history) {
...
function getCorrectionHint(lastUserMessage) {
    const t = normalizeText(lastUserMessage);
    if (t.includes('paperback'))
        return 'paperback';
    if (t.includes('hardcover'))
        return 'hardcover';
    if (t.includes('isbn'))
        return 'ISBN';
...
function buildContextAwareReply(args) {
    ...
    if (userQuestion) {
        ...
        if (askingPrice && known?.price) {
            ...
            return {
                ...
                interruptionHandled: Boolean(args.previousState === 'PRODUCT_DISCOVERY' && args.intent !== 'product_search'),
                toneLeadUsed,
...
//# sourceMappingURL=context-aware-reply.util.js.map