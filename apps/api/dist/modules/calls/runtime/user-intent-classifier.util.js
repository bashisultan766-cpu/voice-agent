...
function hasProductOrCatalogSignal(t) {
    return (/\b(do you have|have you got|got any|carry|stock|in stock|isbn|sku|book|books|title|author|edition|copy|looking for|search|find|catalog|check\s+[\w-]+)\b/i.test(t) || /\b\d{10,13}\b/.test(t));
}
function classifyUserIntent(text) {
    ...
    if (!t)
        return 'unclear';
    if (/\b(what store is this|where am i calling|who are you|what is this store|is this a bookstore)\b/.test(t)) {
        return 'store_identity_question';
    }
    if (/\b(do you sell|can i get|do you have)\b/.test(t) &&
    ...
    if (/^(thanks|thank you|thx|ty|appreciate it|much appreciated)\b/i.test(t) ||
    ...
    if (/\b(no|nope|not)\b.*\b(mean|meant|actually)\b|\bi mean\b|\b(instead|rather)\b|\bwrong (book|one|title|item)\b|\bnot that\b|\bdifferent (book|one|title)\b/i.test(t)) {
        return 'correction';
    }
    if (/\b(payment|pay|checkout|how does payment work|secure payment|payment link|how it works)\b/i.test(t)) {
...
//# sourceMappingURL=user-intent-classifier.util.js.map