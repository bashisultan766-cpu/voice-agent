...
const voice_latency_analyzer_service_1 = require("../../calls/runtime/voice-latency-analyzer.service");
const voice_latency_breakdown_util_1 = require("../../calls/runtime/voice-latency-breakdown.util");
...
const realtime_voice_flags_util_1 = require("../../realtime-voice/config/realtime-voice-flags.util");
...
const voice_single_reply_pipeline_util_1 = require("../../calls/runtime/voice-single-reply-pipeline.util");
...
const telephony_spelling_capture_util_1 = require("../../calls/runtime/telephony-spelling-capture.util");
...
let TwilioWebhookService = TwilioWebhookService_1 = class TwilioWebhookService {
    constructor(config, agentResolution, callsService, callEvents, voiceRuntime, sessionContext, transcriptBuffer, elevenLabs, ttsCache, voicePromptAudio, voiceLatencyAnalyzer, prisma, encryption, streamMetrics, voiceCost, streamingSession, elevenStreaming) {
        this.config = config;
    ...
    onModuleInit() {
        ...
        void this.warmPhraseAudioAtStartup().catch((err) => {
            this.logger.warn(`Phrase audio warm skipped: ${err instanceof Error ? err.message.slice(0, 120) : 'unknown'}`);
        });
    }
    async warmPhraseAudioAtStartup() {
        const apiKey = this.config.get('ELEVENLABS_API_KEY')?.trim();
        ...
        const warmResults = await Promise.allSettled(agents.map(async (agent) => {
    ...
    shortenGreetingForCapture(text, maxMs) {
        const t = text.trim();
        if (!t)
            return "I'm still here. How can I help you with your book order?";
    ...
    logVoiceProviderEnforced(callSessionId, route) {
        const policy = this.voiceProviderPolicy();
        if (!policy.twilioSayBlocked)
            return;
        this.logger.log(JSON.stringify({
            ...(0, voice_provider_policy_util_1.buildVoiceProviderEnforcedLog)(policy),
            callSessionId,
            route,
        }));
    }
    resolveGatherHearingDebugEffective() {
        if (this.voiceProviderPolicy().twilioSayBlocked)
            return false;
    ...
    async resolveShortPhrasePlayUrl(params) {
        ...
        const sessionMeta = sessionRow.metadata && typeof sessionRow.metadata === 'object' && !Array.isArray(sessionRow.metadata)
            ? sessionRow.metadata
            : {};
        const blocked = (0, voice_single_reply_pipeline_util_1.shouldBlockNonOrchestratorTts)({
    ...
    async handleInboundVoice(payload) {
        ...
        const inboundGreetingText = (0, book_sales_voice_util_1.resolveInboundGreetingText)(context.agent.greetingMessage);
        ...
        const twiml = this.finalizeTwiml((0, gather_mvp_twiml_1.buildInboundGatherMvpTwiML)({
            ...
            openingSayText: !strictElOnlyInbound && (hearingDebugEffective || !greetingPlaybackUrl)
                ? inboundGreetingText
                : undefined,
            finalFallbackAudioUrl: hearingDebugEffective ? undefined : finalFallbackAudioUrl,
            finalFallbackSayText: strictElOnlyInbound || hearingDebugEffective || finalFallbackAudioUrl ? undefined : fallbackText,
            timeoutSeconds: 5,
            speechTimeout: 'auto',
    ...
    async handleGatherMvpVoice(payload) {
        ...
        const hearingDebugEffective = this.resolveGatherHearingDebugEffective();
        const strictElevenLabsOnly = this.isStrictElevenLabsOnly();
        const speechGate = (0, gather_speech_gate_util_1.computeGatherSpeechGate)({
        ...
        if (!hasUsableSpeech) {
            ...
            await this.transcriptBuffer.append(callSessionId, 'system', !speechText
                ...
                : `No meaningful speech from Twilio Gather (reason=${speechGate.rejectReason ?? 'unknown'}).`, seq);
            if (nextRetry >= 2) {
            ...
            this.logger.log(JSON.stringify({
                ...
                spellingModeActive: metadata.voiceMode === 'SPELLING_CAPTURE' || metadata.orderState === 'EMAIL_COLLECTING',
            }));
            ...
            if (ackSelection.mode === 'sync_full_reply') {
                const latencyTimer = new voice_latency_breakdown_util_1.VoiceLatencyTimer();
                latencyTimer.startSection('intentDetectionMs');
                const instantReplyText = (0, instant_reply_util_1.buildInstantReply)(speechText, ctx.store?.name ?? 'SureShot Books');
                const instantAudioPhrase = (0, instant_reply_util_1.instantReplyAudioPhrase)(speechText, ctx.store?.name ?? 'SureShot Books');
                ...
                const syncGatherTiming = (0, telephony_spelling_capture_util_1.resolveGatherTwiMLOptions)(metadata, {
                    speechTimeout: '2',
                    timeoutSeconds: 10,
                    pauseBeforeListenSeconds: 0,
                });
                const useTwilioSayFallback = !mainPlay.playbackUrl && !strictElevenLabsOnly && !this.blockTwilioSay();
            ...
            const kickText = ackSelection.mode === 'deferred_kickoff' ? (ackSelection.instantPhrase?.trim() ?? '') : '';
            ...
            if (kickText.length > 0) {
                ...
                if (cachedKick.playbackUrl) {
                    ...
                    kickPhrase = {
                        ...
                        voiceProviderActuallyUsed: (0, voice_provider_policy_util_1.resolveVoiceProviderActuallyUsed)(true, this.voiceProviderPolicy()),
                    };
                }
                else {
        ...
        const gatherTiming = (0, telephony_spelling_capture_util_1.resolveGatherTwiMLOptions)(metadata, {
            speechTimeout: '2',
            timeoutSeconds: 10,
            pauseBeforeListenSeconds: 0,
        });
    ...
    async handleDeferredVoicePoll(payload) {
        ...
        const meta = row.metadata && typeof row.metadata === 'object' && !Array.isArray(row.metadata)
            ? row.metadata
            : {};
        const deferredGatherTiming = (0, telephony_spelling_capture_util_1.resolveGatherTwiMLOptions)(meta, {
            speechTimeout: '2',
            timeoutSeconds: 10,
            pauseBeforeListenSeconds: 0,
        });
        ...
        if (job.phase === 'processing') {
            ...
            const fillerThresholdMs = (0, voice_commerce_fast_mode_util_1.voiceSearchFillerThresholdMs)();
            const voicePolicyPoll = this.voiceProviderPolicy();
            if (elapsed > fillerThresholdMs &&
                !job.momentPromptPlayed &&
                (0, voice_provider_policy_util_1.shouldPlayDeferredSearchFiller)(voicePolicyPoll)) {
                await this.callsService.mergeSessionMetadata(callSessionId, {
                    deferredVoiceJob: { ...job, momentPromptPlayed: true },
                });
                const lastIntent = typeof meta.lastIntentDetected === 'string'
                    ? meta.lastIntentDetected
                    : 'product_search';
                const fillerText = (0, voice_search_filler_util_1.pickVoiceSearchFillerPhrase)({
                    callSessionId,
                    intent: lastIntent,
                    queryPreview: typeof meta.lastProductQuery === 'string' ? meta.lastProductQuery : undefined,
                });
                ...
                const fillerPlay = cachedFiller.playbackUrl
                    ? {
                        ...
                        voiceProviderActuallyUsed: (0, voice_provider_policy_util_1.resolveVoiceProviderActuallyUsed)(true, voicePolicyPoll),
                    }
        ...
        const twiml = this.finalizeTwiml((0, gather_mvp_twiml_1.buildInboundGatherMvpTwiML)({
            ...
            finalFallbackSayText: strictElevenLabsOnly || finalFallbackAudioUrl ? undefined : playbackAudioUrl ? undefined : gatherFallbackText,
            timeoutSeconds: deferredGatherTiming.timeoutSeconds,
            speechTimeout: deferredGatherTiming.speechTimeout,
            pauseBeforeListenSeconds: deferredGatherTiming.pauseBeforeListenSeconds,
            blockTwilioSay: this.blockTwilioSay(),
    ...
    async executeDeferredVoiceJobBody(callSessionId, speechText, jobId) {
        ...
        try {
            ...
            this.logger.log(JSON.stringify({
                ...
                ttsGenerationTimeMs,
                totalVoiceTurnLatencyMs,
                fastMode: (0, voice_commerce_fast_mode_util_1.isVoiceCommerceFastMode)(),
            }));
            const row = await this.callsService.findOneById(callSessionId);
            const meta = row.metadata && typeof row.metadata === 'object' && !Array.isArray(row.metadata)
                ? row.metadata
            ...
            await this.callsService.mergeSessionMetadata(callSessionId, {
                ...(0, voice_single_reply_pipeline_util_1.buildLlmReplyMetadataPatch)(assistantResponse),
                deferredVoiceJob: {
                    jobId,
                    phase: 'ready',
                    startedAtMs: cur.startedAtMs,
            ...
            const deferredBreakdown = this.voiceLatencyAnalyzer.buildDeferredBreakdown({
                ...
                turnProof: {
                    ...(utter.turnProof ?? {}),
                    openaiSkippedReason: typeof proof?.skipOpenAiGeneration === 'boolean' && proof.skipOpenAiGeneration
    ...
    async buildElevenLabsPlaybackUrl(publicOrigin, text, opts) {
        ...
        const ttsStart = Date.now();
        try {
            const speechText = (0, voice_elevenlabs_playback_util_1.prepareVoiceTtsInputText)(text);
            if (!speechText) {
                this.logger.warn(JSON.stringify({
            ...
            const ttsResult = await this.voicePromptAudio.createPhrasePlaybackUrl(publicOrigin, {
                text: speechText,
                voiceId,
                apiKey: opts.elevenlabsApiKey ?? this.config.get('ELEVENLABS_API_KEY') ?? undefined,
                callSessionId: opts.callSessionId,
            });
